...
public class InterfaceAddress {
    ...
    private void mapComponentInfo() {
        ...
        component = map(supplier, x -> x.isSubComponent() ? x.getParentIndex() : x.getIndex());
        subComponent = map(supplier, InterfaceComponent::getSubComponentIndex);
    }
    ...
    public String toShortString() {
        if (component == -1) {
            InterfaceComposite composite = InterfaceComposite.valueOf(root);
            if (composite != null) {
                return composite.toString() + "<" + root + ">";
            }
        }
    ...
    private static class SupplyOnce<K> implements Supplier<K> {
